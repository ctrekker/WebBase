(function(){
    ...
    window.onload=function() {
        ...
        //signInForm.setAttribute("action", "/app/signin.php");
        ...
        //signUpForm.setAttribute("action", "/app/signup.php");
        ...
        window.signInContent=signInContent;
        window.signUpContent=signUpContent;

        window.detectLinks();

        //Set course-background-image to no drag
        var courseBackgrounds=document.getElementsByClassName("course-background-image");
        for(var i=0; i<courseBackgrounds.length; i++) {
            var background=courseBackgrounds[i];
            ...
            links[i].addEventListener("mousedown", function(e) {
                ...
                if(url=="signin") {
                    signInButtons=[
                        ...
                        ["Sign In", function(e) {
                            ajax.post("/app/signin.php", {
                                siUsername: getText("siUsername"),
                                siPassword: getText("siPassword"),
                                ajax: "true"
                            }, function(data) {
                                if(data=="COMPLETE") window.location="/app/home/";
                                else window.popup("Unable to sign in", (function() {
                                    var div=document.createElement("div");
                                    div.appendChild(document.createTextNode(data));
                                    return div;
                                }()), undefined, undefined, window.popup.DEFAULT_ALERT);
                            });
                            //document.getElementsByClassName("signin-form")[0].submit();
                        }]
                    ];
                    window.popup("Sign In", window.signInContent, 350, 150, signInButtons);
                }
                else if(url=="signup") {
                    signUpButtons=[
                        ["Cancel", window.popup.DESTROY_METHOD, true],
                        ["Make Account", function(e) {
                            ajax.post("/app/signup.php", {
                                suFirst: getText("suFirst"),
                                suLast: getText("suLast"),
                                suEmail: getText("suEmail"),
                                suUsername: getText("suUsername"),
                                suPassword: getText("suPassword"),
                                suPasswordC: getText("suPasswordC"),
                                ajax: "true"
                            }, function(data) {
                                if(data=="COMPLETE") window.location="/app/home/";
                                else window.popup("Unable to sign up", (function() {
                                    var div=document.createElement("div");
                                    div.appendChild(document.createTextNode(data));
                                    return div;
                                }()), undefined, undefined, window.popup.DEFAULT_ALERT);
                            });
                            //document.getElementsByClassName("signup-form")[0].submit();
                        }]
                    ];
                    window.popup("Sign Up", window.signUpContent, 415, 243, signUpButtons);
                }
                else if(url=="NONE") {
                    //DO NOTHING!!!
                }
